...
model_weight_name = 'LWNet_49'  # todo 模型名称
...
def pridict(val_loader, model, params, class_names):
    ...
    with torch.no_grad():  # 开始推理
        for i, (images, target) in enumerate(stream, start=1):
            ...
            ROC_y_preds.append(torch.softmax(output, dim=1).cpu().numpy())
            ROC_real_labels.append(target.cpu().numpy())
            ...
            precision = calculate_precision_macro(output, target)  # 计算precision
            # specificity = calculate_specificity_macro(output, target)  # 计算precision
            ...
            # metric_monitor.update('Specificity', specificity)
    ...
    show_heatmaps( x_labels=class_names, y_labels=class_names, harvest=heat_maps_float,
                  save_name=save_name) #=============================================================================================

    ROC_y_preds = np.concatenate(ROC_y_preds)
    ROC_real_labels = np.concatenate(ROC_real_labels)


    # 加上模型名称

    return metric_monitor.metrics['Accuracy']["avg"], metric_monitor.metrics['F1']["avg"], \
           metric_monitor.metrics['Recall']["avg"],metric_monitor.metrics['Precision']["avg"],\
        ROC_y_preds, ROC_real_labels


def show_heatmaps( x_labels, y_labels, harvest, save_name):
    # 这里是创建一个画布
    fig, ax = plt.subplots()
...
if __name__ == '__main__':
    ...
    acc, f1, recall ,precision,y_preds, test_real_labels= pridict(test_loader, model, params, class_names)
    print("测试结果：")
    ...
    for i in range(len(class_names)):
        fpr[i], tpr[i], _ = roc_curve(test_real_labels == i, y_preds[:, i])
        roc_auc[i] = auc(fpr[i], tpr[i])

    # Plot ROC curves
    plt.figure(figsize=(7.5, 5))
    colors = ['blue', 'red', 'green', 'purple', 'orange', 'brown', 'pink', 'gray','Gold']
    ...
    # plt.title('{}'.format("vit_base_patch16_224_in21k"))#=============================================================================================
    plt.legend(loc="lower right")
    plt.grid(True)
    plt.tick_params(axis='both', labelsize=14)  # Set tick labels' font size
    save_name = "result_img/{}/ROC_{}.png".format(model_name, model_name)
    with open(save_name, 'wb') as f:
        plt.savefig(save_name, dpi=100)
    plt.show()