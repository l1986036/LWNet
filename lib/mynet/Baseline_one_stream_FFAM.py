...
class GateFusion(nn.Module):
    ...
    def forward(self, x0, x2,x1):
        # 融合低频和高频特征
        fusion1 =x0+x2
        fusion2 =x2+x1
        x = self.conv1(torch.cat((fusion1, fusion2), dim=1))
        # 分割通道
...
class ATT(nn.Module):
    ...
    def __init__(self, in_channel, out_channel):
        ...
        self.global_att = global_module(in_channel)
        self.conv1 = nn.Conv2d(in_channel, out_channel, kernel_size=1)

    def forward(self, x, x_boun_atten):
        out1 = x + x_boun_atten
        global_att = self.global_att(out1)

        transpose = torch.transpose(x, 2, 3)

        out = transpose.mul(global_att)
...
class Baseline_one_stream_FFAM(nn.Module):
    # resnet based encoder decoder
    def __init__(self, num_classes):
        super(Baseline_one_stream_FFAM, self).__init__()
    ...
    def forward(self, xx):
        ...
        ## -------------------------------------- ##
        low_x = self.low_fusion(x0, x2,x1)#24×112×112
        ...
        cat_out_01 = self.cat_01(low_x, low_x.mul(low_x))
        cat_out_01 = self.downSample(cat_out_01) #48×56×56

        cat_out11 = self.cat_11(cat_out_01, conv1.mul(conv1))
        cat_out11 = self.downSample(cat_out11) #196×28×28

        cat_out21 = self.cat_21(cat_out11, conv2.mul(conv2))
...
if __name__ == "__main__":
    ...
    net = Baseline_one_stream_FFAM(8).cuda()
    print(cal_param(net))
